    ...
    artistList.innerHTML = artist.albums
      .map(
        (album) => `
                    <div id="accordion-${
                      album.id
                    }" class="border border-gray-700 rounded-lg overflow-hidden mb-4">
                        <h2 id="heading-${album.id}">
                            <button type="button" class="accordion-button flex items-center justify-between w-full p-5 bg-gray-800 text-white font-medium text-left focus:outline-none" data-accordion-target="#body-${
                              album.id
          ...
          album.id
                                ...
                                <span class="flex items-center">
                                    <img src="${
                                      album.cover || "default-album-cover-url"
                                    }" alt="${
          album.title
        }" class="album-cover rounded w-16 h-16 object-cover mr-4" />
                                    ${album.title}
                                </span>
                        ...
                        <div id="body-${
                          album.id
                        }" class="hidden accordion-content p-5 bg-gray-700 text-gray-400 transition-max-height max-h-0" aria-labelledby="heading-${
          album.id
        }">
